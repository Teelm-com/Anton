...
if(!is_user_logged_in()){
	wp_redirect(get_permalink(aotonhemes_page("template/login.php")));
	exit;
}
$is_submit_page = 1;
date_default_timezone_set('Asia/Shanghai');
$post_tougao_role = 'read';
...
if(isset($_POST['security']) && is_user_logged_in()){
	if($security_nonce == $_POST['security']){
	    ...
	    if ( $post_tougao_time && (time() - strtotime($last_post) < $post_tougao_time*60) ){
			...
			$title =   $wpdb->_escape($_POST['title']) ;
			$content =  $_POST['content'] ;
			$cat =  $wpdb->_escape($_POST['cat']);
			$status = 'pending';
			$submit = array(
			...
			if ($status != 0) {
				...
				if($_POST['image']){
					update_post_meta($status,'_thumbnail_ext_url',$wpdb->_escape($_POST['image']));
				}
		    		...
		    		<div class="tougao-content">
		    			<form method="post">
		    				...
		    				<div class="tougao-item">
		    					<label>分类 *</label>
		    					<div>
			    					<div class="tougao-select"><?php wp_dropdown_categories('show_option_all=选择分类&orderby=name&hierarchical=1&selected=-1&depth=0&hide_empty=0');?></div> <div class="tougao-tax"></div>
			    				</div>
		    				</div>
		    				<div class="tougao-item">
		    					<label>封面图</label>
		    					<div class="tougao-image-wrap clearfix">
			    					<div class="tougao-image-box tougao-upload" title="上传图片"><i class="uil-image-plus"></i><br>上传图片</div>
